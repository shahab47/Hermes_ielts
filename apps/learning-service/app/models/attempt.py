...
class Attempt(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    prompt_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("tasks.id"), nullable=True)
    ...
    audio_asset: Mapped[Optional["AudioAsset"]] = relationship(back_populates="attempt", uselist=False)
...
class Assessment(Base, UUIDPrimaryKeyMixin):
    ...
    criterion_scores: Mapped[list["CriterionScore"]] = relationship(back_populates="assessment", cascade="all, delete-orphan")
