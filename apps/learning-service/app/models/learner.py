...
if TYPE_CHECKING:
    ...
    from app.models.task import PracticeSession, Recommendation
...
class Learner(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    target_overall_band: Mapped[float | None] = mapped_column(Float, nullable=True)
    exam_date: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    timezone: Mapped[str] = mapped_column(String(50), default="UTC")
    baseline_status: Mapped[BaselineStatus] = mapped_column(Enum(BaselineStatus), default=BaselineStatus.PENDING)
...
class SkillState(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    recent_score: Mapped[float | None] = mapped_column(Float, nullable=True)
    rolling_score: Mapped[float | None] = mapped_column(Float, nullable=True)
